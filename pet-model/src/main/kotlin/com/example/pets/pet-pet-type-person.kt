...
data class Person(
  ...
  @TestOnly operator fun <T> invoke(imitation: () -> T): T = imitation()

  val pets: List<Pet>
    get() = listOf(*ownsPets.toTypedArray())

  val petTypes: Bag<PetType>
    get() = pets.groupBy { it.type }.mapValues { it.value.size }

  val numberOfPets: Int
    get() = pets.size

  val isPetPerson: Boolean
    get() = pets.isNotEmpty()
